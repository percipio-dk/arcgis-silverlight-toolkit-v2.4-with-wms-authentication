...
namespace ESRI.ArcGIS.Client.Toolkit
{
    ...
    public partial class EditorWidget : Control
    {
		...
		private void UpdateVisibleButtons()
		{
			...
			bool atLeastOneLayerNotReadOnly = isVisible;
			...
			foreach (GraphicsLayer layer in this.editor.GraphicsLayers)
			{
				...
				if (layer is FeatureLayer)
				{
					...
					if (!hasPolygonOrPolyline && !flayer.IsReadOnly && flayer.Visible && flayer.LayerInfo != null &&
					...
					if (!atLeastOneLayerNotAutoSave && !flayer.AutoSave)
						atLeastOneLayerNotAutoSave = true;
					if (!atLeastOneLayerNotReadOnly && !flayer.IsReadOnly && flayer.Visible)
						atLeastOneLayerNotReadOnly = true;
				}
				else if (layer.Visible)
					hasPolygonOrPolyline = true; //GraphicsLayer may contain polygon or polyline
			}
			bool allButtonsDisabled = !hasPolygonOrPolyline && !atLeastOneLayerNotAutoSave && !atLeastOneLayerNotReadOnly
			&& !atLeastOneLayer && !atLeastOneFeatureLayer;
			DisableOrHideElement(TemplatePicker, atLeastOneLayerNotReadOnly, allButtonsDisabled);
			DisableOrHideElement(EditGeometry, hasPolygonOrPolyline || atLeastOneLayerNotReadOnly, allButtonsDisabled);
			...
			DisableOrHideElement(DeleteSelect, atLeastOneLayerNotReadOnly, allButtonsDisabled);
			...
			DisableOrHideElement(Options, hasPolygonOrPolyline, allButtonsDisabled);
		}
