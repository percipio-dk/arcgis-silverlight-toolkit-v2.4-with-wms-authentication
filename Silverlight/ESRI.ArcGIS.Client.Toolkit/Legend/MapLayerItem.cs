...
namespace ESRI.ArcGIS.Client.Toolkit
{
	...
	internal sealed class MapLayerItem : LayerItemViewModel
	{
		...
		{
			if (!(sender is GroupLayer)) // For group layers, we don't wait for initialized event 
				Refresh();
		}
		...
		internal void Refresh()
		{
			...
			if (!(Layer is GroupLayer)) // GroupLayer : don't wait for layer intialized, so the user will see the layer hierarchy even if the group layer is not initialized yet (else would need to wait for all sublayers initialized)
			{
