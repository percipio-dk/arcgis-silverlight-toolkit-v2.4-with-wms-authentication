...
namespace ESRI.ArcGIS.Client.Toolkit.Primitives
{
	...
	public class LayerItemViewModel : LegendItemViewModel
	{
		...
		private void LayerItem_PropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
		{
			...
			else if (e.PropertyName == "IsTransparent")
			{
				LayerItemsSourceChanged();
			}
		...
		private IEnumerable<LegendItemViewModel> GetLayerItemsSource()
		{
			...
			// Enumerates LayerItems
			if (_layerItems != null)
			{
				foreach (LayerItemViewModel layerItem in _layerItems)
				{
					if (!LayerItemsOptions.ShowOnlyVisibleLayers || layerItem.IsVisible)
					{
						bool hasLayerChildren = layerItem.LayerItems != null && layerItem.LayerItems.Any(item => !item.IsTransparent);
		...
		internal struct LayerItemsOpts
		{
			internal LayerItemsOpts(
				...
				bool returnLegendItems, bool showOnlyVisibleLayers)
			{
		...
		internal void UpdateLayerVisibilities(bool isParentVisible, bool isParentInScaleRange)
		{
			bool isEnabled;
			...
			// A layer is visible if it's checked on, if all its ascendants are visible and if it's in scale range
			bool isVisible = _isEnabled && isParentVisible && IsInScaleRange;
			...
			LayerItems.ForEach(layerItem => layerItem.UpdateLayerVisibilities(IsVisible, IsInScaleRange));
		}
